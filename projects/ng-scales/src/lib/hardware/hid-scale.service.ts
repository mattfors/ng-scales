...
export class HidScaleService implements HardwareScaleInterface {
  ...
  constructor(private hidScaleMapperService: HidScaleMapperService, private navigatorService: NavigatorService) {
  ...
  open(): Observable<void> {
    ...
    return this.navigatorService.requestDevice(SUPPORTED_SCALES)
      .pipe(
  ...
  private start(d: HIDDevice): void {
    ...
    this.subscriptions.add(
      this.navigatorService.addHidEventListener<HIDInputReportEvent>('disconnect')
        .pipe(
